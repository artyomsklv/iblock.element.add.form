...
if ($arParams["USE_SECTION_PROPERTY"] == "Y") {
	$resElement = CIBlockElement::GetList(array("SORT" => "ASC"), array("ID" => $arParams["ID"]));
	if ($curElement = $resElement->Fetch())
	{
		foreach ($arParams["SECTION_".$curElement["IBLOCK_SECTION_ID"]."_PROPERTY_CODES"] as $sectionPropId) {
			$arParams["PROPERTY_CODES"][] = $sectionPropId;
		}
	}
}
