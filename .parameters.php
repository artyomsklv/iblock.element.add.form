...
if(!CModule::IncludeModule("iblock"))
	...
	if(($arCurrentValues["IBLOCK_ID"] > 0) && ($arCurrentValues["USE_SECTION_PROPERTY"] == "Y")) {
		$arSectionList = array();
		$arSectionSelect = Array('ID', 'NAME');
		$arSectionFilter = Array('IBLOCK_ID'=>$arCurrentValues["IBLOCK_ID"], 'ACTIVE'=>'Y', 'GLOBAL_ACTIVE'=>'Y');
		$res = CIBlockSection::GetList(Array('SORT'=>'ASC'), $arSectionFilter, true, $arSectionSelect);
	...
	$arProperty_LNSF = array(
		"ACTIVE" => GetMessage("IBLOCK_ADD_ACTIVE"),
	...
	$arComponentParameters = array(
		...
		"PARAMETERS" => array(
			...
			"USE_SECTION_PROPERTY" => array(
				"PARENT" => "FIELDS",
				"NAME" => GetMessage("IBLOCK_USE_SECTION_PROPERTY"),
				"TYPE" => "CHECKBOX",
				"REFRESH" => "Y",
			),
	...
	if(!empty($arCurrentValues['USE_SECTION_PROPERTY'])) {
		foreach($arSectionList as $k => $v) {
			$arComponentParameters["PARAMETERS"]["SECTION_".$v['ID']."_PROPERTY_CODES"] = array(
				"PARENT" => "FIELDS",
				"NAME" => GetMessage('IBLOCK_PROPERTY_SECTION').'"'.$v['NAME'].'"',
				"TYPE" => "LIST",
				"MULTIPLE" => "Y",
				"ADDITIONAL_VALUES" => "N",
				"VALUES" => $arProperty_LNSF,
